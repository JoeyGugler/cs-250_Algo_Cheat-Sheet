...
\usepackage[flushbottom=false]{multicol}
...
\end{align*}
\textbf{Complexity:} \\
Time: \( \Theta(n^{\log_2 7}) \approx \Theta(n^{2.81}) \), Space: \( \Theta(n^2) \)
\endtcolorbox
...
\textbf{Operations:}
\begin{itemize}[noitemsep,itemsep=0pt]
    \item \textbf{Search:} Find an element with a specific key — \(\Theta(n)\)
    \item \textbf{Insert:} Insert an element at the head — \(\Theta(1)\)
    \item \textbf{Delete:} Remove an element — \(\Theta(1)\)
\end{itemize}
...
\begin{algorithmic}[1]
    ...
    \Statex \Comment{Runs in $O(h)$ time, where $h$ is the height of the tree}
...
\textbf{Counting Sort} assumes the input consists of $n$ integers in the range $0$ to $k$ and sorts them in $\mathcal{O}(n + k)$ time. It is stable and non-comparative.
...
\textbf{Time complexity:} $\mathcal{O}(mn)$ \quad
\textbf{Space complexity:} $\mathcal{O}(mn)$
\endtcolorbox
...
\textbf{Runtime:}
\begin{itemize}[noitemsep, topsep=0pt]
    \item $\Theta(|V|^2)$ with a simple array
    \item $\Theta((|V| + |E|)\log |V|)$ with a binary heap
\end{itemize}
\endtcolorbox
